#!/usr/bin/env python3
"""Check for Python files that won't be included in documentation.

This script detects files that would be excluded from documentation builds due to
missing `__init__.py` files in parent directories.
...
src = root / "src" / "alberto_codes_site"
...
def check_docs_coverage(files: list[Path], src_root: Path) -> list[CoverageIssue]:
    ...
    Args:
        ...
        src_root: Root of the source tree (e.g., src/alberto_codes_site)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Check for Python files excluded from documentation"
    )
    parser.add_argument(
        "--files",
        nargs="*",
    ...
    parser.add_argument(
        "--src-root",
        default="src/alberto_codes_site",
    ...
    print(f"Found {len(issues)} file(s) excluded from documentation build:\n")
