...
def check_griffe_warnings(files: list[str] | None = None) -> list[str]:
    ...
    Args:
        files (list[str] | None): Optional list of files to check.
            If None, checks all of src/alberto_codes_site.
    ...
    try:
        # Load the package
        pkg = griffe.load(
            "alberto_codes_site", search_paths=["src"], docstring_parser="google"
        )
    ...
    if files:
        filtered = []
        for w in warnings:
            # Warning format: "src/alberto_codes_site/api.py:1098: No type..."
            if ":" in w:
                warn_file = w.split(":")[0]
...
def main() -> int:
    ...
    parser.add_argument(
        "--files",
        nargs="*",
        help="Specific files to check (default: all src/alberto_codes_site)",
    )
